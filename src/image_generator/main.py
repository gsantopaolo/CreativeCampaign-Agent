...
async def generate_image(creative_request: creative_generate_pb2.CreativeGenerateRequest):
    ...
    try:
        ...
        # Get locale-specific creative brief from campaign
        creative_brief_key = f"creative_brief_{locale}"
        creative_brief = campaign.get("localization", {}).get(creative_brief_key, "")
        
        # Base creative content (generated by LLM in locale language)
        creative_content = creative.get('content', '')
        headline = creative.get('headline', '')
        
        logger.info(f"  📝 Using locale-specific data for {locale}:")
        logger.info(f"     Creative Brief: {creative_brief_key}")
        logger.info(f"     Headline: {headline}")
        
        # Generate images for ALL aspect ratios with CUSTOMIZED prompts
        generated_images = []
        ...
        for aspect_ratio in aspect_ratios:
            ...
            # Build aspect-ratio-specific prompt
            # Each aspect ratio gets a different composition optimized for its format
            composition_guidance = {
                "1x1": "centered composition with balanced symmetry, product prominently displayed in the middle third, equal spacing on all sides",
                "4x5": "vertical portrait composition with product in upper two-thirds, elegant vertical flow, space at bottom for text overlay",
                "9x16": "tall vertical story format with product positioned in center or upper portion, dramatic vertical composition, generous bottom space for text",
                "16x9": "wide horizontal landscape composition with product positioned left or right of center, cinematic wide-angle view, space for text on opposite side"
            }
            
            aspect_guidance = composition_guidance.get(aspect_ratio, "balanced composition")
...
"""
            ...
            logger.info(f"  🤖 Calling OpenAI DALL-E {OPENAI_IMAGE_MODEL} with {aspect_ratio}-optimized prompt...")
            
            # Call DALL-E with aspect-ratio-specific prompt
...
async def main():
    ...
    enable_probe = os.getenv("IMAGE_GENERATOR_ENABLE_READINESS_PROBE", "true").lower()
    ...
    if enable_probe in ("true", "yes", "1"):
        ...
        logger.info("ℹ️ Readiness probe disabled via IMAGE_GENERATOR_ENABLE_READINESS_PROBE")
